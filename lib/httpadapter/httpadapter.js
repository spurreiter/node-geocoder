...
*/
const HttpAdapter = function(http, options) {
...
HttpAdapter.prototype.get = function(url, params, callback) {
  ...
  const urlParsed = this.url.parse(url);
  const options = {
  ...
  const request = this.http.request(options, function(response) {
    let str = "";
  ...
  const onError = function(err) {
    const error = err instanceof HttpError ? err : new HttpError(err.message, {
      code: err.code
    });
    callback(error, null);
  };
